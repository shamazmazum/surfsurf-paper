...
whole range $[0, f]$. In our work we propose $a = 0.5$ and $\xi = 0.1$ as
...
immediately observe that the images on top (with resolution 4096x4096 and
1024x1024 and surface CFs very close to analytical solution) satisfy our
$C_{0.5}$ criterion perfectly. The other two downscaled images fail to satisfy
the criterion. As we can see in conjunction from \cref{fig:scaling} and
...
sandstone and three carbonate samples with dimensions 500x500x500
(\cref{fig:real-data}) all of which satisfy our $C_{0.5}$ criterion. The result
can be seen on \cref{fig:real-data-plots}.
