...
\begin{figure}
  ...
  \caption{Stages of the algorithm for computation of surface functions.}
  \label{fig:stages}
\end{figure}
...
\begin{figure}[ht]
  ...
  \caption{Surface-surface correlation function calculated with different
    interface extraction methods (Sobel to the left, distance map to the
    right).}
...
to detect edges of the image, so $M = S$ in algorithms in \cref{sec:general}. We
...
non-periodic boundary conditions we will need a minor change.
...
\begin{figure*}[t]
  \centering
  \subfigure[Resolution $4096 \times 4096$, value noise, $C_{0.5} = 0.9692$]{
    \includegraphics[width=0.22\linewidth]{images/noise-4096.png}
    \label{fig:noise-4096}}
  \hfill
  \subfigure[Resolution $1024 \times 1024$, value noise, $C_{0.5} = 0.9386$]{
    \includegraphics[width=0.22\linewidth]{images/noise-1024.png}
    \label{fig:noise-1024}}
  \hfill
  \subfigure[Resolution $256 \times 256$, value noise, $C_{0.5} = 0.8770$]{
    \includegraphics[width=0.22\linewidth]{images/noise-256.png}
    \label{fig:noise-256}}
  \hfill
  \subfigure[Resolution $64 \times 64$, value noise, $C_{0.5} = 0.7524$]{
  ...
  \subfigure[Resolution $64 \times 64$, disk radius 0.96 pixels, $C_{0.5} = 0.6549$]{
    ...
    \label{fig:disks-64}}
  \caption[]{Images of value noise and overlapping disks with different
    resolution (\cref{sec:results}).}
...
of about 12000 pixels for each disk. A natural question arises: ``Is there a
...
to establish such empirical criterion based on spectral analysis of input images
and \cref{sec:crit} provides necessary details.
...
thesholded value noise with three octaves is on
...
\subsection{Effects of image upscaling}
...
nearest-neighbor. At \cref{fig:resized} you can see images of overlapping disks
...
and image filtering -- have an error compared to the analytical
solution. \textcolor{red}{How is that?? In my opinion Sobel is pretty close to
  the theory.} If we increase the radius of disks and the image resolution
...
satisfy our $C_{0.5}$ criterion perfectly. The other three downscaled images
fail to satisfy the criterion. The first two images of value noise (with
resolution $4096 \times 4096$ and $1024 \times 1024$) pass our criterion and the
other two do not pass, which is in agreement with \cref{fig:fss-scaling-noise} -
\cref{fig:fsv-scaling-noise} (correlation functions converge at resolution about
$1024 \times 1024$).

After looking at \cref{fig:disks-noise} and \cref{fig:scaling-alltogether} we
can claim that the criterion correctly selects images which are suitable for
calculation of the correlation functions. The criterion $C_{0.5}$ decays fast
with the decline in image resolution as shown in \cref{fig:crit-plot}.
...
For demonstration purposes, we computed correlation functions for XCT images of
three sandstone and three carbonate samples with dimensions
$500 \times 500 \times 500$ all of which satisfy our $C_{0.5}$ criterion. The
result can be seen on \cref{fig:real-data-xct}.

Also we computed correlation functions for SEM images of two sandstones
(with dimensions $1024 \times 691$) and two carbonates ($1280 \times 869$). The
result is on \cref{fig:real-data-sem}.
...
\begin{figure*}[t]
  ...
  \caption[]{Examples of SEM images along with surface correlation functions
    calculated using our method.}
...
\begin{figure}[t]
  ...
  \caption[]{Examples of images which do not pass $C_{0.5}$ criterion with
    $C_{0.5} < 0.9$.}
  \label{fig:real-bad}
\end{figure}
